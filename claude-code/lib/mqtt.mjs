...
// Auth matches the device firmware: username = deviceId
// (RUBATO-<mac6>), password = the per-unit token from the provisioning ledger.
// The Claude Code daemon identifies itself with clientId CC-RUBATO-<mac6>
// (registered CC prefix, spec §2.2, acting on the mac6 part of the device
// id), distinct from the device's own clientId (= deviceId), so the two
// clients never kick each other.
...
export async function publishRecord(config, record) {
  // clientId goes to the broker exactly as configured (CC-RUBATO-<mac6>) — no
  // extra suffix. It differs from the device's own clientId (= deviceId), so
  // the two clients never kick each other. Note: only ONE Rubato daemon
  ...
  // over the session. The daemon lock file enforces the single instance.
