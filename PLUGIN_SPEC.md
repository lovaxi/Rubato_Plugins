...
- **插件**：寄生在智能体宿主进程内，观察每一次流式模型调用，产出状态消息并发布 MQTT。**本地归档（§5）、时长预估与校准（§3）都是 dsh 的开发期设施**——其他插件（用户端）除配置外零本地落盘、零模型接口。
...
| OpenCode | `OC-` | 在用 |
| Cursor | `CUR-`（设备贴纸 `Cursor-RUBATO-<mac6>`，clientId = `CUR-Cursor-RUBATO-<mac6>`） | 在用 |
| OpenClaw | `Claw-`（`Claw-` + username = `Claw-RUBATO-<mac6>`） | 在用 |
| Claude Code | `CC-RUBATO-`（作用于 mac6：`CC-RUBATO-<mac6>`） | 在用 |
| Codex | `Codex-`（`Codex-` + username = `Codex-RUBATO-<mac6>`） | 在用 |
...
7. **归档与 wire 分离（仅 dsh）**：`publish(record, wire = record)`——归档永远收完整对象，MQTT 只发精简对象。Done 是唯一当前需要分离的消息。其他插件无归档，直接发精简 wire。
...
## 3. 时长预估器（kNN，零 token）——**dsh 先行，测试完成后统一添加到其他插件**

预估在每次调用启动时完成，不消耗任何 token，随样本积累自动变准。**预估与校准由 dsh 先行验证**：在 dsh 测试完成前，其他插件不维护校准样本、不输出 estSec（设备健康提醒随之只在 dsh 会话中触发）；dsh 验证通过后，按本节规范统一添加到其他插件。
...
| `l` | 最后一条**含真实文本**的 user 消息长度 | 工具结果续接消息虽是 user 角色但无文本部件——必须跳过，否则 l/f/v/fl 在工具循环的每次续接调用上全部饿死（真实踩坑：75% 的调用特征为空）|
...
归档是 **dsh 开发环境的诊断设施**；用户端插件除配置外**零本地落盘**，排障依赖设备端与 broker 控制台。
...
- mqmon_status 是给 **AI 智能体**用的诊断面板：注册进宿主 tools 服务，模型可在对话中直接调用，回答"凭据配了吗 / 往哪发 / 发了多少 / 最近一次成功吗"。dsh 运行在 AI 开发环境里，这类工具有用；**其他插件运行在用户端，尽量简洁——不注册任何模型工具，也不写本地归档（§5）**，排障依赖设备端与 broker 控制台。
...
- wire 精简 / 归档完整分离；
- clientId 前缀登记（§2.2）、config 查找顺序、首启 UX、console 政策全部照抄 dsh；**模型工具（§6）与本地归档（§5）仅 dsh**——用户端插件零模型接口、零本地落盘；**预估器暂不移植**（§3：待 dsh 测试完成后统一添加）。
...
| 数据文件名 | `rubato-records.jsonl` / `rubato-stats.json` **均仅 dsh**（旧名 `thinktime-*` 首次触达自动迁移，见 §5）；其他插件零本地落盘 |
...
- [ ] 本地归档与 stats：**仅 dsh**（§5）——其他插件零本地落盘，不复制此设施
