...
export async function publishRecord(config, record) {
  // clientId goes to the broker exactly as configured (Claw-Rubato-<mac6>) — no
  // extra suffix. It differs from the device's own clientId (= deviceId),
  // so the two clients never kick each other. Note: only ONE publishing
  // process may run at a time — the Serverless front-end rejects a duplicate
  // clientId with CONNACK 2 instead of taking over the session.
