# Thinktime — DeepSeek Harness 插件

[English](README.md) | 简体中文

Thinktime 让 AI 的工作状态在桌面实体设备上可见。本目录是 DeepSeek Harness
（DSH）插件部分；设备固件在
[lovaxi/Thinktime_Device](https://github.com/lovaxi/Thinktime_Device)。

```
DSH 模型调用 ──> Thinktime 插件 ──> EMQX 服务器 ──> Thinktime 设备（TFT 屏幕）
```

每次流式模型调用都会以 MQTT 消息发布：
...
主题：`thinktime/<deviceId>/state`——每台设备一个主题。
...
这就是全部安装步骤——插件包就是该仓库的 `dsh/` 目录。其余由助手完成并提醒你重启。重启后：
