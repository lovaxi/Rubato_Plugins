// Rubato MQTT publisher for the Cursor plugin — persistent in-process MQTT
// 3.1.1 connection, ported 1:1 from the dsh/ plugin (authoritative:
// dsh/lib/mqtt.js) to CommonJS so the extension host loads it without a build
// step. One TCP/TLS connection is kept alive (PINGREQ keepalive) and reused
// for every record, so each publish is a single small packet instead of a
...
async function publishRecord(config, record) {
  ...
  // with CONNACK 2 instead of taking over the session.
