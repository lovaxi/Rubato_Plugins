...
钩子则保持不动），并注册 + 启动**无窗口**的开机登录计划任务
`RubatoCodexWatcher` 运行 watcher——桌面上不会出现任何控制台窗口。

> **注意**：watcher 以 `node.exe` 进程完全隐形地后台运行（无任何窗口），它
> 就是观察 Codex、驱动设备的那部分，必须保持运行。退出或重启 Codex
> 应用/CLI 完全没关系；只是不要在任务管理器里杀掉 `node.exe`/`wscript.exe`。
> 万一 watcher 真的死了，下一次 Codex 回合的 `notify` 钩子会把它拉起来并解卡
> 设备。
